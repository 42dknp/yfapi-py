...
class HistoricDataValidatorTest(unittest.TestCase):
    ...
    def test_validate_results(self):
        ...
        del data_missing['chart']['result'][0]['meta']['currency']
        with self.assertRaises(Exception) as context:
            HistoricDataValidator.validate_results(data_missing)
        self.assertEqual(str(context.exception), 'Missing currency property')

    def test_validate_currency(self):
        data_missing_currency = self.data_object
        del data_missing_currency['chart']['result'][0]['meta']['currency']
        with self.assertRaises(Exception) as context:
            HistoricDataValidator.validate_results(data_missing_currency)
        self.assertEqual(str(context.exception), 'Missing currency property')

    def test_validate_symbol(self):
        data_missing_symbol = self.data_object
        del data_missing_symbol['chart']['result'][0]['meta']['symbol']
        with self.assertRaises(Exception) as context:
            HistoricDataValidator.validate_results(data_missing_symbol)
        self.assertEqual(str(context.exception), 'Missing symbol property')

    def test_validate_exchange_name(self):
        data_missing_exchange_name = self.data_object
        del data_missing_exchange_name['chart']['result'][0]['meta']['exchangeName']
        with self.assertRaises(Exception) as context:
            HistoricDataValidator.validate_results(data_missing_exchange_name)
        self.assertEqual(str(context.exception), 'Missing exchangeName property')

    def test_validate_price(self):
        data_missing_price = self.data_object
        del data_missing_price['chart']['result'][0]['meta']['regularMarketPrice']
        with self.assertRaises(Exception) as context:
            HistoricDataValidator.validate_results(data_missing_price)
        self.assertEqual(str(context.exception), 'Missing regularMarketPrice property')

    def test_missing_meta_property(self):
        data_missing = self.data_object
        del data_missing['chart']['result'][0]['meta']
        with self.assertRaises(Exception):
            HistoricDataValidator.validate_results(data_missing)

    def test_missing_quote_properties(self):
        data_missing = self.data_object
        del data_missing['chart']['result'][0]['indicators']['quote']
        with self.assertRaises(Exception):
            HistoricDataValidator.validate_results(data_missing)

    def test_validate_quote_properties(self):
        data_missing = self.data_object
        del data_missing['chart']['result'][0]['indicators']['quote'][0]['volume']
        with self.assertRaises(Exception) as context:
            HistoricDataValidator.validate_results(data_missing)
        self.assertEqual(str(context.exception), 'Invalid sub-properties for volume')

    def test_validate_adjclose_properties(self):
        data_missing = self.data_object
